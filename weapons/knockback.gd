...
## Gets the velocity at a amount in time_elapsed adjusted
## by a curve if one exists in the knockback stast	
func get_velocity() -> Vector2:
	var velocity = direction * stats.force
	var time_elapsed_secs = (Time.get_ticks_msec() - time_start_msecs) / 1000
	var percentange_elapsed = time_elapsed_secs / stats.duration
		
	if stats.curve:
		var sample_value = stats.curve.sample(percentange_elapsed)
